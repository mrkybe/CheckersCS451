...
namespace Data
{
    ...
    public class CheckersGM
    {
        ...
        private void MovePiece(Point from, Point to)
        {
            ...
            State endState = nFrom.GetState();
            if (nTo.GetSparsePosition().Y == 0 || nTo.GetSparsePosition().Y == 7)
            {
                if (endState == State.RED)
                {
                    endState = State.KING_RED;
                }
                else if (endState == State.BLACK)
                {
                    endState = State.KING_BLACK;
                }
            }
